...
object Crawler extends Crawler {
  ...
  : Fv[T] = {
    ...
    (actor ? Get(urlStr, timeout, retry, interval) map {
  ...
  case class Get(urlStr: String, timeout: Timeout, retry: Retry, interval: Interval)

  class CrawlActor extends Actor {

    def receive = {
      case Get(urlStr, _, Retry(r), _) if r <= 0 => sender ! ConnectionError(urlStr).failure
      case Get(urlStr, Timeout(t), Retry(r), Interval(i)) => {
        val resultOpt = allCatch opt {
          val future = http(url(urlStr) OK as.Bytes)
          Await.result(future, t seconds)
        }
        resultOpt match {
          case Some(bodyBytes) => sender ! bodyBytes.success
          case None => self.tell(Get(urlStr, Timeout(t), Retry(r - 1), Interval(i)), sender)
        }
        if (i > 0)
          Thread.sleep(i)
      }
